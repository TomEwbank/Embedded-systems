...
#define PING_RECEIVED_LOW 472
#define PING_RECEIVED_HIGH 552
...
volatile int tmr0_overflow_counter = 0;
//volatile int tmr1_overflow_counter = 0;
volatile int tmr1_counter_low = 0;
volatile int tmr1_counter_high = 0;
//volatile int tmr1_counter = 0;
//volatile char tmr1_counter_low_bis = 0;
//volatile char tmr1_counter_high_bis = 0;
...
void interrupt interrupt_service_routine(void) {    
    ...
    if (INTCONbits.TMR0IE && INTCONbits.TMR0IF) { 
        ...
        if (listening) {
            if (tmr0_overflow_counter == 35) {
                tmr0_overflow_counter = 0;
                send_ping = true;
                listening = false;
            ...
            ping_send = true;
            send_ping = false;
        } else if (blocking_sensors && tmr0_overflow_counter == 15) { // Blocked during 15ms
            tmr0_overflow_counter = 0;
...
//        ++tmr1_overflow_counter;
    ...
    if (PIE1bits.ADIE && PIR1bits.ADIF) {
        ...
        if (sensor1_val < PING_RECEIVED_LOW || sensor1_val > PING_RECEIVED_HIGH) {
            stopTimer1();
            tmr1_counter_low = TMR1L;
            tmr1_counter_high = TMR1H;
            LED_PORT2 = !LED_PORT2;
//            tmr1_counter = tmr1_counter_low + (tmr1_counter_high << 8); 
            RTT_received = true;
            ADIE = 0;
        }
         startADC();
...
//    int fetched_tmr1_overflow_counter = -1;
//    int fetched_tmr1_counter = -1;
//    int f_tmr1_counter_low = 0;
    int f_tmr1_counter_high = 0;
//    char f_tmr1_counter_low_bis = 0;
//    char f_tmr1_counter_high_bis = 0;
//    int distance = -1;
...
//                fetched_tmr1_overflow_counter = tmr1_overflow_counter;
//                fetched_tmr1_counter = tmr1_counter;
//                f_tmr1_counter_low = tmr1_counter_low;
                f_tmr1_counter_high = tmr1_counter_high;
//                f_tmr1_counter_low_bis = tmr1_counter_low_bis;
//                f_tmr1_counter_high_bis = tmr1_counter_high_bis;
                
//                long RTT = fetched_tmr1_overflow_counter*16384+(fetched_tmr1_counter>>2)-17320;
//                double d = RTT*34/200.0;
//                distance = d;
            ...
            if (readyForUART && notSentToUART) {
                //send_debug("---------");
                //send_coord(distance, 0);
                //send_coord(fetched_tmr1_overflow_counter, fetched_tmr1_counter);
                send_coord(f_tmr1_counter_high, 0);
                //send_coord(f_tmr1_counter_high_bis, f_tmr1_counter_low_bis);
                readyForUART = false;
                notSentToUART = false;
            }
//            ++c;
